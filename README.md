...
- **Re-run `update-checkout` after every `git pull` of `swift/`.** It pins the sibling repos
  ...
  keeping the workspace internally consistent so the siblings move *with* swift. Run it the
...
links. If it still dies, **add swap**, which turns OOM-death into "slow but finishes":
...
2. **The just-built (unwrapped) clang that compiles the stdlib** knows no NixOS paths.
   ...
   delivered via `CPLUS_INCLUDE_PATH`, because that injects like `-isystem`, which `-nostdinc++`
...
6. **C++ interop (`CxxStdlib`)** has two coupled problems, both solved in `flake.nix` +
   `dobuild.sh`:

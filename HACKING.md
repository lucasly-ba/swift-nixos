...
- `$SWIFT_CORELIBS_SDK`: an *augmented* sysroot (glibc + the just-built Swift runtime) for
  the Foundation build. `$SWIFT_GCC_LIB` / `$SWIFT_RUNTIME_LIB` are the gcc-lib / core-runtime
...
3. **What it revealed.** Main thread waiting on a worker; the worker in `SILVerifier`, inside
   ...
   interface.
4. **The fixes** (all to the **corelibs only**, via `--common-swift-flags`, and never the
...
2. **The clean Foundation fix.** Make the build compile `swift-syntax` with the just-built
   6.5-dev compiler so its binary modules load directly, and then *none* of the §3 corelibs
...
- The **executable** test suite (build + run, `import StdlibUnittest`): ~99% (`Interpreter`
  257/260), via the flake's `SWIFT_DRIVER_TEST_OPTIONS`, with no manual setup.
...
2. **`--debug-swift` (assert) compiler only.** We build the debug/asserts compiler (correct for
   contributing to `swift/lib` + stdlib, since you *want* assertions). A release build is untested here;
